...
\input{sections/figs/fig_method}
...
% - 段 1：整体 roadmap——DINO-SPR = DINOv3 + spatiotemporal 适配（SSTS 空间 + KPTF
%   时序），DINO-SPR Lite = architecture-agnostic 蒸馏（CAP + STGD）。模块只用缩写带过，
...
To overcome label scarcity in few-shot surgical phase recognition, we introduce \textsc{DINO-SPR}, unlocking DINOv3's~\cite{simeoni2025dinov3} generalization for the surgical domain.
As illustrated in Figure~\ref{fig:method}, we first devise a spatiotemporal adaptation tuning that elevates independent frame-level features into sequence representations via SSTS and KPTF modules. By explicitly filtering the inherent redundancy of surgical videos to aggregate only the most discriminative spatiotemporal signals, this design imposes a strong inductive bias crucial for few-shot learning.
For resource-constrained clinical deployment, we further distill it into \textsc{DINO-SPR Lite}, an architecture-agnostic student where CAP bridges representational gaps and STGD enriches sparse supervision with the teacher's priors.

% ════════════════════════════════════════════════════════════════════


% ════════════════════════════════════════════════════════════════════
\subsection{Spatiotemporal Adaptation Tuning}  % → DINO-SPR (Phase 1 teacher)
...
agree.
Since a patch that stays in between is penalised whichever side of $\mathcal{S}_t$ it falls on,
...
SSTS scores each patch with its spatial neighbourhood and the frame context, suppresses the background
through a soft top-$K$, and fuses the patches into a frame token, one frame at a time.
% ────────────────────────────────────────────────────────────────────
...
%     过 temporal attention（TAttn，标准 MHA+FFN+LN 一个 block）→ 取 clip 位置输出为 clip summary ĥ。
...
%   融合：线性投影 f_fuse 拼接合并 ĥ 与 v（[ĥ; v] 过 Linear(2D→D)），输出 clip 特征 h̃。
...
%     可学习 clip token h 聚合跨帧上下文，clip 位置输出即 clip summary ĥ（D 维）。
...
%   融合：$\tilde{\mathbf{h}} = f_{\mathrm{fuse}}([\hat{\mathbf{h}}; \mathbf{v}])$ 线性投影合并两路（[ĥ; v] concat 后 Linear(2D→D)），无门控。
...
%   - 输出：取 clip 位置输出（[0]，D 维）当 clip summary ĥ，作为整段摘要。
...
%
%   两路融合：线性投影 f_fuse 拼接合并 temporal token ĥ（支路一 eq8）与关键帧增强 token v（支路二，正文内联）
%
%   - 线性投影：拼接 [ĥ; v]（2D 维）过 Linear(2D→D) 一次投影出 clip 特征 h̃，无门控、无残差插值。
%
%   输出
%
...
the frames that differ markedly from their neighbours carry the discriminative evidence.
The keyframe-scoring branch rates each frame by its inter-frame difference
$\boldsymbol{\delta}_t = \tilde{\mathbf{c}}_t - \tilde{\mathbf{c}}_{t-1}$, so that a frame of marked
...
small set of parameters is trained, the LoRA adapters on the query/value projections of the
...
% ════════════════════════════════════════════════════════════════════
\subsection{Architecture-Agnostic Distillation}  % → DINO-SPR Lite (Phase 2 student)
\label{sec:method:distill}

% 引入（承上启下 + 架构依赖问题 + 三大家族 + 两个设计模块，均不写具体数字）：
% - 承上启下：DINO-SPR 做 spatiotemporal 适配后继承了 DINOv3 的泛化、few-shot 表现好，
%   但仍是 architecture-dependent（绑死 DINOv3 ViT 形态）。
% - 架构依赖的缺点：部署端灵活度不足、参数量大加载缓慢、推理效率低。
% - 因此引入 architecture-agnostic 蒸馏 → DINO-SPR Lite：可蒸馏到任意架构轻量 student，
%   达到部署友好 + 实时推理。
...
%       使蒸馏对 student 架构无关；
%   (2) STGD 时空引导蒸馏：few-shot 下蒸馏信号 scarce，用 DINO-SPR 的 spatial/temporal
%       prior 引导 student，提升蒸馏效率。

DINO-SPR inherits the generalization of DINOv3 and delivers promising few-shot
...
We therefore distill it into \textsc{DINO-SPR Lite}, whose backbone can follow any mainstream
...
% 动机：稀疏。标签太少 → 用 DINO-SPR 的 4 个 spatiotemporal 中间信号去弥补，
...
It keeps a set of learnable queries in the teacher's space, one for every token the student owes the
teacher, a patch query for each of the teacher's patches plus a single class query for the class slot
the student lacks.
...
Both are normalized over their support, so each enters the KL as a distribution.
...
The student is trained with the same tuning objective as the teacher with the two
distillation terms,
\begin{equation}
